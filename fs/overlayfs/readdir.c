...
static int ovl_dir_fsync(struct file *file, struct dentry *de, int datasync)
{
	struct ovl_dir_file *od = file->private_data;
...
//	return vfs_fsync_range(od->realfile, od->realfile->f_path.dentry, start, end, datasync);
	return vfs_fsync(od->realfile, od->realfile->f_path.dentry, datasync);
}
